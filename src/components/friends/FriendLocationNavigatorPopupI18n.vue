...
const textSource = {
  noUpdate: { zh: '暂无更新时间', en: 'No update time yet', ja: '更新時刻はまだありません', ko: '아직 업데이트 시간이 없습니다' },
  eyebrow: { zh: '好友定位详情', en: 'Friend Location Details', ja: '友だちの位置詳細', ko: '친구 위치 상세' },
  online: { zh: '在线', en: 'Online', ja: 'オンライン', ko: '온라인' },
  offline: { zh: '离线', en: 'Offline', ja: 'オフライン', ko: '오프라인' },
  sharingOn: { zh: '位置共享已开启', en: 'Location sharing on', ja: '位置共有オン', ko: '위치 공유 켜짐' },
  sharingOff: { zh: '位置共享未开启', en: 'Location sharing off', ja: '位置共有オフ', ko: '위치 공유 꺼짐' },
  loadingMap: { zh: '正在加载好友位置...', en: 'Loading friend location...', ja: '友だちの位置を読み込み中...', ko: '친구 위치를 불러오는 중...' },
  mapHint: { zh: '这里会显示好友最近一次共享的位置。', en: 'The friend’s latest shared location will appear here.', ja: 'ここに友だちが最後に共有した位置が表示されます。', ko: '여기에 친구가 마지막으로 공유한 위치가 표시됩니다.' },
  mapUnavailable: { zh: '当前无法显示地图', en: 'Map is currently unavailable', ja: '現在地図を表示できません', ko: '지도를 표시할 수 없습니다' },
  latitude: { zh: '纬度', en: 'Latitude', ja: '緯度', ko: '위도' },
  longitude: { zh: '经度', en: 'Longitude', ja: '経度', ko: '경도' },
  accuracy: { zh: '精度', en: 'Accuracy', ja: '精度', ko: '정확도' },
  updatedAt: { zh: '最近更新', en: 'Last Update', ja: '最終更新', ko: '최근 업데이트' },
  walking: { zh: '步行', en: 'Walking', ja: '徒歩', ko: '도보' },
  driving: { zh: '驾车', en: 'Driving', ja: '車', ko: '차량' },
  routeSummary: { zh: '找朋友路线', en: 'Route to Friend', ja: '友だちのもとへ行くルート', ko: '친구에게 가는 경로' },
  routePending: { zh: '点击下方按钮后，会根据你的当前位置规划路线。', en: 'Tap below to plan a route from your current location.', ja: '下のボタンを押すと現在地から友だちまでのルートを作成します。', ko: '아래 버튼을 누르면 현재 위치를 기준으로 경로를 안내합니다.' },
  routeFailed: { zh: '路线规划失败，请稍后再试。', en: 'Failed to plan the route. Please try again later.', ja: 'ルートの作成に失敗しました。しばらくしてから再試行してください。', ko: '경로 계획에 실패했습니다. 잠시 후 다시 시도해 주세요.' },
  locationRequired: { zh: '需要先允许浏览器获取你的位置，才能规划去找好友的路线。', en: 'Allow browser location access to plan a route.', ja: '友だちのもとへ行くルートを作るには、先にブラウザの位置情報を許可してください。', ko: '친구에게 가는 경로를 만들려면 먼저 브라우저 위치 권한을 허용해야 합니다.' },
  navigate: { zh: '导航去找 TA', en: 'Navigate', ja: '会いに行く', ko: '찾아가기' },
  refreshRoute: { zh: '刷新路线', en: 'Refresh Route', ja: 'ルートを更新', ko: '경로 새로고침' },
  close: { zh: '关闭', en: 'Close', ja: '閉じる', ko: '닫기' },
  friendLocationLabel: { zh: '好友位置', en: 'Friend Location', ja: '友だちの位置', ko: '친구 위치' },
  myLocationLabel: { zh: '我的位置', en: 'My Location', ja: '現在地', ko: '내 위치' },
};
...
const fallbackMessage = computed(() => (
  props.errorMessage
  || getFriendLocationFallbackMessage(props.friend, language.value)
...
function renderFriendMarker(AMap) {
  ...
  friendMarker = new AMap.Marker({
    ...
    title: props.friend?.username || text.value.friendLocationLabel,
    label: {
      content: buildLabel(props.friend?.username || text.value.friendLocationLabel),
      direction: 'top',
    },
  });
...
function renderCurrentMarker(AMap) {
  ...
  currentMarker = new AMap.Marker({
    ...
    title: text.value.myLocationLabel,
    offset: new AMap.Pixel(-10, -24),
    label: {
      content: buildLabel(text.value.myLocationLabel, 'friend-map-label--self'),
      direction: 'top',
    },
  });
...
<template>
  <Popup :show="show" round position="bottom" teleport="body" class="location-popup" @update:show="handleVisibleChange">
    <div v-if="friend" class="popup-content">
      ...
      <div class="map-card">
        <div ref="mapRoot" class="map-root" :class="{ 'is-hidden': !canRenderMap }" />
      ...
      <section v-if="canRenderMap" class="route-card">
        <div class="route-card-head">
          ...
          <div class="route-mode-switch">
            <button type="button" class="route-mode-btn" :class="{ 'is-active': routeMode === 'walking' }" @click="routeMode = 'walking'">
              {{ text.walking }}
            </button>
            <button type="button" class="route-mode-btn" :class="{ 'is-active': routeMode === 'driving' }" @click="routeMode = 'driving'">
              {{ text.driving }}
            </button>
          </div>
        </div>
        ...
        <div v-else-if="routeSteps.length" class="route-step-list">
          <article v-for="(step, index) in routeSteps" :key="`${index}-${step}`" class="route-step-item">
